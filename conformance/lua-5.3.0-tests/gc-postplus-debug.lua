...
collectgarbage(); print("GC_CALL", debug.getinfo(1,"l").currentline)
...
if not _port then
  ...
  collectgarbage(); collectgarbage(); print("GC_CALL", debug.getinfo(1,"l").currentline)
  local x = gcinfo()
  collectgarbage"stop"
  assert(not collectgarbage("isrunning"))
  repeat
...
print("BEFORE_ERROR_PCALL"); assert(not pcall(collectgarbage)); print("AFTER_ERROR_PCALL")
...
print("MARK_AFTER_USERDATA_SKIP")
-- __gc x weak tables
...
u, m = nil
collectgarbage(); print("GC_CALL", debug.getinfo(1,"l").currentline)
print("BEFORE_M_ASSERT", m); assert(m==10)
